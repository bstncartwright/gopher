...
func TestMatrixAttachmentResolverFromExplicitAttachmentField(t *testing.T) {
	workspace := t.TempDir()
	...
	attachments := resolver("!dm:local", "writer", sessionrt.Event{
		...
		Payload: map[string]any{
			"name":   "write",
			"status": "ok",
			"result": map[string]any{"attachments": []any{"report.md"}},
		},
	})
...
func TestMatrixAttachmentResolverSkipsWriteWithoutExplicitAttachmentField(t *testing.T) {
	workspace := t.TempDir()
	target := filepath.Join(workspace, "report.md")
	if err := os.WriteFile(target, []byte("# report\n"), 0o644); err != nil {
		t.Fatalf("os.WriteFile() error: %v", err)
