...
func buildToolUsageHints(registry ToolRegistry) string {
	...
	if toolRegistryHas(registry, "delegate_targets") {
		lines = append(lines, "- `delegate_targets` lists the local and remote delegation targets currently available from this agent. Use it before `delegate` when you need to inspect valid `target_agent` choices.")
	}
...
func buildCollaborationSection(input systemPromptInput) string {
	...
	if toolRegistryHas(input.Tools, "delegate_targets") {
		lines = append(lines, "Use `delegate_targets` when you need the current list of local or remote delegation targets before choosing `target_agent`.")
	}
	return strings.Join(lines, "\n")
}
