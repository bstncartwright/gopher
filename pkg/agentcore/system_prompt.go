...
func buildCollaborationSection(input systemPromptInput) string {
	...
	if delegateEnabled && len(known) > 1 {
		lines = append(lines, "When delegation helps, use `delegate` with `action:\"create\"` and a task-specific `message`; provide `target_agent` when you want a specific worker and `model_policy` to override an ephemeral worker model. `create` is async spawn: do not block this turn with `sleep`/poll loops; resume delegated follow-up only after a later `delegation.completed` or `delegation.failed` event.")
	} else if delegateEnabled {
		lines = append(lines, "Delegation can auto-create ephemeral subagents in a single-agent runtime. Use `delegate` `action:\"create\"` with a task-specific `message`; optionally set `target_agent` to pick or name a worker and optionally set `model_policy` to override the ephemeral worker model. `create` is async spawn: do not block this turn with `sleep`/poll loops; resume delegated follow-up only after a later `delegation.completed` or `delegation.failed` event.")
	} else if len(known) > 1 {
