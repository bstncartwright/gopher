...
func TestStreamOpenAIResponsesAutoFallsBackAfterNormalWebSocketClosure(t *testing.T) {
	var wsAttempts atomic.Int32
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		events := []map[string]any{
			...
			{"type": "response.output_item.done", "item": map[string]any{"type": "message", "id": "msg_1", "role": "assistant", "status": "completed", "content": []any{map[string]any{"type": "output_text", "text": "Recovered"}}}},
			{"type": "response.completed", "response": map[string]any{"status": "completed", "usage": map[string]any{"input_tokens": 4, "output_tokens": 2, "total_tokens": 6, "input_tokens_details": map[string]any{"cached_tokens": 0}}}},
		}
	...
	if len(result.Content) == 0 || result.Content[0].Text != "Recovered" {
		t.Fatalf("expected text Recovered, got %#v", result.Content)
	}
	if wsAttempts.Load() == 0 {
		t.Fatalf("expected at least one websocket attempt before SSE fallback")
	}
